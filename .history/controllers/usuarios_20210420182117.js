...
const usuariosGet = async(req = request, res = response ) => {
     ...
     const usuarios = await Usuario.find({ estado: true }) // <== Me traigo solo los que tengas el estado true
          .skip( Number( desde) ) // <== indica desde donde quieres los datos. 
          .limit(Number( limite) ); // <== indica la cantidad de datos que te regrese. 

    const total = await Usuario.countDocuments({ estado: true }); //<== Obtienes el total de valores 
